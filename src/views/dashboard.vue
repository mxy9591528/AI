    ...
    <el-row style="margin-top: 20px;">
      <el-card style="width: 100%">
        ...
        <div class="ai-provider-settings">
          <el-radio-group v-model="aiProvider.provider" @change="handleProviderChange">
            <el-radio-button label="bailian">阿里云百炼（云端）</el-radio-button>
          ...
          <template v-if="aiProvider.provider === 'ollama'">
            ...
            <el-tag v-if="!aiProvider.ollamaOnline" type="danger" style="margin-left: 15px;">Ollama 未运行</el-tag>
          </template>
        </div>
      </el-card>
    </el-row>
...
<script setup>
import { getAnalyticsOverview, getAiProvider, switchAiProvider } from '@/api/admin'
...
const loadAiProvider = () => {
  getAiProvider().then(res => {
    ...
    aiProvider.value.model = res.ollamaModel
...
const handleProviderChange = (val) => {
  switchAiProvider({ provider: val, model: aiProvider.value.model }).then(res => {
    ...
    aiProvider.value.models = res.ollamaModels || []
    aiProvider.value.ollamaOnline = res.ollamaOnline
    ElMessage.success('AI 模型已切换')
  })
}

const handleModelChange = (val) => {
  switchAiProvider({ provider: 'ollama', model: val }).then(() => {
    ElMessage.success('Ollama 模型已切换为 ' + val)
  })
}
